...
class Net(nn.Module):
    ...
    def forward(self, data=None, **variables):
        ...
        variables = {k: convert_to_gpu_if_enabled(torch.autograd.Variable(torch.from_numpy(v.copy())) if numpy else v)
...
class Layer(torch.autograd.Function):
    ...
    def backward(self, grad_outputs):
        ...
        self.caffe_python_layer.backward(top, self.caffe_propagate_down, bottom) # 这里的blob改为了b
        return tuple(convert_to_gpu_if_enabled(torch.from_numpy(v.diff.contents.reshape(*v.reshape)))
                     if propagate_down else None for v, propagate_down in zip(bottom, self.caffe_propagate_down))
...
def to_dict(obj):
    ...
    f.name: converter(v) if f.label != FD.LABEL_REPEATED else list(map(converter, v)) for f, v in obj.ListFields() for
    converter in [{FD.TYPE_DOUBLE: float, FD.TYPE_SFIXED32: float, FD.TYPE_SFIXED64: float, FD.TYPE_SINT32: int,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', dest='output_path',
                        help='Path to converted model, supported file extensions are: h5, npy, npz, json, pth')
